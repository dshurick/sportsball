...
poissonfit <- function(dtf) {
  ...
  # fit <- brm(points ~ (1 | away_team) + (1 | home_team) + 1,
  #            family = negbinomial(),
  #            data = fitdf)
  # 
  # fit <- brm(points ~ away_team + home_team:home_adv2,
  #            family = negbinomial(),
  #            data = fitdf)
  # 
  ...
  glmnetGrid <-
    expand.grid(lambda = exp(seq(-2.22, -0.78, length.out = 121)),
                alpha = 0.5762711864) # seq(0, 1, length.out = 60))
  ...
  coef(poisson_fit$finalModel, s = 0.239788019)[, 1]
