...
train_data <- games %>%
  filter(Season < opt$year, Yes >= 0.2 |
           CRType == 'NCAA')
...
glm_gridperf2 <- h2o::h2o.getGrid(grid_id = "glm_grid2",
                                  sort_by = "logloss",
                                  decreasing = FALSE)
...
best_glm_perf2 <- h2o::h2o.performance(model = best_glm2,
                                       newdata = test_hdf)

submission_hdf <-
  h2o::as.h2o(submission, destination_frame = "submission_hdf")

y_hat <- predict(best_glm2, submission_hdf)
